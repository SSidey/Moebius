...
a `moeb run` or `moeb spec` invocation. Skill files are mutable harness documents and are
not subject to the immutability policy. The default skills (`run.skill.md`,
...
| Patch File Tool | Adds a patch_file tool that accepts a unified diff and applies it to an existing file via the diffy crate, reducing write payloads to only changed lines; subject to the same read-before-write scope enforcement as write_file and prerequisite for sub-agent spawning | [specifications/moeb/moeb.patch-file-tool.md](specifications/moeb/moeb.patch-file-tool.md) | active |
| Sub-Agent Spawning | Introduces a spawn_agent tool that delegates bounded file-analysis tasks to isolated sub-agent loops returning unified diffs; coordinator applies diffs via patch_file, maintaining coordinator-only write invariant and context isolation per sub-task | [specifications/moeb/moeb.sub-agent-spawning.md](specifications/moeb/moeb.sub-agent-spawning.md) | superseded |
| Split Agent: Extract Inner Loop | Extracts run_agent_loop_inner, continue_nudge, and truncate from agent.rs into a new agent_inner.rs companion module, reducing agent.rs from 331 to ~100 lines and eliminating it from the context-budget exceptions allowlist | [specifications/moeb/moeb.split-agent.md](specifications/moeb/moeb.split-agent.md) | active |
| Split Domain Spec: Parser and Schema Extraction | Extracts parse_frontmatter, validate_sections, sanitize_slug, and REQUIRED_SECTIONS into spec_parser.rs, and ValidationSchema types with load_validation_schema into spec_schema.rs, reducing spec.rs from 492 to ~286 lines | [specifications/moeb/moeb.split-domain-spec.md](specifications/moeb/moeb.split-domain-spec.md) | active |
| Split Anthropic Adapter: Request and Response Extraction | Extracts build_request_body, build_messages, and to_anthropic_message into anthropic_request.rs, and parse_response and parse_tool_call into anthropic_response.rs, reducing anthropic.rs from 421 to ~255 lines | [specifications/moeb/moeb.split-anthropic-adapter.md](specifications/moeb/moeb.split-anthropic-adapter.md) | active |
| Split OpenAI Adapter: IO Extraction and Test Separation | Extracts to_openai_message, to_openai_tool, parse_response, and parse_tool_call into openai_io.rs, and moves the inline test block to openai_tests.rs, reducing openai.rs from 364 to ~238 lines | [specifications/moeb/moeb.split-openai-adapter.md](specifications/moeb/moeb.split-openai-adapter.md) | active |
| Split Replay Command: Adapter, Executor, and Test Separation | Extracts ReplayAiAdapter and parse_response_body into replay_adapter.rs, ReplayToolExecutor into replay_executor.rs, and the inline test block into replay_tests.rs, reducing replay.rs from 390 to ~203 lines | [specifications/moeb/moeb.split-replay-command.md](specifications/moeb/moeb.split-replay-command.md) | active |
| Rubric Context Layers: Binary-Bundled Baseline Criteria per Command Context | Introduces binary-bundled run.rubrics.md and spec.rubrics.md asset files combined with project-specific .moeb/rubrics/ files and injected via {{command_rubrics}}; baseline run contains only context-budget (a refactor trigger), build/test criteria live in project-specific run.rubrics.md, spec baseline contains no-drift and spec-schema-compliance | [specifications/moeb/moeb.rubric-context-layers.md](specifications/moeb/moeb.rubric-context-layers.md) | active |
| AI-First Organisation Principles | Formalises four AI-first organisation principles (one concern per file, context locality, grep-discoverable names, no cross-cutting helpers) as a general harness policy in assets/README.md and adds the ai-first-org criterion to the binary-bundled spec baseline rubric | [specifications/moeb/moeb.ai-first-principles.md](specifications/moeb/moeb.ai-first-principles.md) | active |
| MCP Stdio Server | Adds a `moeb serve` command that runs moeb as a stdio MCP server, exposing all standard file and task-management tools via JSON-RPC 2.0 over stdin/stdout so Claude Desktop (Claude Pro subscription) can drive a spec implementation without a moeb API key | [specifications/moeb/moeb.mcp-stdio-server.md](specifications/moeb/moeb.mcp-stdio-server.md) | active |
| MCP HTTP + OAuth Server | Extends `moeb serve` with `--transport http` mode implementing the MCP Streamable HTTP transport and an embedded OAuth 2.0 + PKCE authorisation server, enabling claude.ai web (Claude Pro) to connect to moeb as a remote MCP server via an external HTTPS terminator such as Cloudflare Tunnel | [specifications/moeb/moeb.mcp-http-oauth-server.md](specifications/moeb/moeb.mcp-http-oauth-server.md) | active |
| Spec Command: Failure Mode Fixes | Fixes three `moeb spec` failure modes: treats empty agent response as retryable, strengthens the closing-delimiter instruction in `spec.prompt`, and prohibits `patch_file` in `readme-link.prompt` | [specifications/moeb/moeb.spec-failure-mode-fixes.md](specifications/moeb/moeb.spec-failure-mode-fixes.md) | active |
| README Link Agent: Enforce Correct Unified Diff Format | Supersedes the patch_file prohibition from moeb.spec-failure-mode-fixes by providing explicit `@@` hunk construction guidance in `readme-link.prompt`, keeping README updates minimal-diff; also fixes the missing Status column in the row template | [specifications/moeb/moeb.readme-link-patch-format.md](specifications/moeb/moeb.readme-link-patch-format.md) | active |
| Patch File Tool: Normalize Hunk Header Counts Before Parsing | Adds a `normalize_hunk_counts` pre-parse step to `patch_file.rs` that recalculates `@@` header counts from actual hunk content, eliminating "Hunk header does not match hunk" parse failures caused by AI arithmetic errors | [specifications/moeb/moeb.patch-file-normalize-hunk-counts.md](specifications/moeb/moeb.patch-file-normalize-hunk-counts.md) | active |
| Patch File Tool: Relocate Hunk Start Positions to Match File Content | Adds `find_closest_context` and `relocate_hunk_positions` to `patch_file.rs`, searching the original file for each hunk's context lines and adjusting `orig_start`/`new_start` by the measured delta before passing the diff to `diffy`, eliminating apply failures caused by wrong start line numbers | [specifications/moeb/moeb.patch-file-context-relocate.md](specifications/moeb/moeb.patch-file-context-relocate.md) | active |
| Spec Integration Tests: Git CWD Isolation | Fixes the spec service integration tests that fail with "Unable to read current working directory" by serialising tests through CWD_LOCK and initialising an isolated git repository in each test's tempdir | [specifications/moeb/moeb.spec-integration-test-git-isolation.md](specifications/moeb/moeb.spec-integration-test-git-isolation.md) | active |
| MCP Serve / CLI Parity | Unifies `moeb serve` and `moeb spec`/`moeb run` CLI by adding VCS tools to the standard registry, making `spec.skill.md` drive file writing/branching/linking/committing, rendering prompt templates with binary rubrics in both `start_run` and a new `start_spec` MCP tool, and removing Rust-side VCS and file-writing from `domain/spec.rs` | [specifications/moeb/moeb.serve-cli-parity.md](specifications/moeb/moeb.serve-cli-parity.md) | active |
| Spec Skill Path Correction: MCP Working Directory Alignment | Corrects spec.skill.md phases 4-7 and spec.prompt to use .moeb/-prefixed paths so that specs are written to .moeb/specifications/ from the repository-root working directory instead of the project root | [specifications/moeb/moeb.spec-skill-path-correction.md](specifications/moeb/moeb.spec-skill-path-correction.md) | active |
| Domain Spec Dead Code Removal | Gates parse_frontmatter, validate_sections, REQUIRED_SECTIONS, and the spec_schema module with #[cfg(test)] to eliminate dead-code warnings in non-test builds while preserving all unit-test coverage | [specifications/moeb/moeb.domain-spec-dead-code-removal.md](specifications/moeb/moeb.domain-spec-dead-code-removal.md) | active |
| Run Command: Post-Completion Git Commit | Extends `git_commit` with a `kind` parameter and adds a Commit phase to `run.skill.md` so that `moeb run` automatically commits all implementation changes with `feat(<domain>): execute <slug> specification` after rubric verification | [specifications/moeb/moeb.run-git-commit.md](specifications/moeb/moeb.run-git-commit.md) | active |
| Run Command: SemVer Version Bump and Candidate Tag | Inserts a Version and Tag phase into `run.skill.md` that classifies the change, bumps `src/moeb/Cargo.toml` per SemVer 2.0.0, commits the bump, and creates an annotated candidate tag `v<version>-candidate.<domain>-<slug>` on the version-bump commit | [specifications/moeb/moeb.run-semver-bump-and-candidate-tag.md](specifications/moeb/moeb.run-semver-bump-and-candidate-tag.md) | active |
| Composable Review Wrapper | Introduces a two-tier quality introspection system for moeb skills: a per-step Writer→Reviewer→Moderator review loop (via a new review_artifact kernel tool) and an end-of-skill QA Architect review that emits structured ReviewSignals in four categories, with run metrics persistence and rolling-average regression detection | [specifications/moeb/moeb.composable-review-wrapper.md](specifications/moeb/moeb.composable-review-wrapper.md) | active |
| Query Agent Tool | Introduces a thin generic `query_agent` kernel tool (role, prompt, expected_response_type, context_files) replacing `spawn_agent` and `review_artifact`; moves all sub-agent coordination into skill instructions; adds kernel-thin-and-parity to global rubric baseline and no-dead-code to run rubric baseline | [specifications/moeb/moeb.query-agent-tool.md](specifications/moeb/moeb.query-agent-tool.md) | active |
| Reviewer and Moderator Role Files | Defines binary-bundled `reviewer.role.md` and `moderator.role.md` persona files required by the Per-Step Review Sub-Loop; each embeds its mandatory output format contract (unified diff and JSON verdict respectively) directly in the role file | [specifications/moeb/moeb.reviewer-and-moderator-roles.md](specifications/moeb/moeb.reviewer-and-moderator-roles.md) | active |
| MCP Serve: Binary Version in Initialize Handshake | Wires `env!("CARGO_PKG_VERSION")` into the `serverInfo.version` field of the MCP `initialize` response for both stdio and HTTP transports, ensuring clients always receive the correct binary version during the protocol handshake | [specifications/moeb/moeb.serve-version-handshake.md](specifications/moeb/moeb.serve-version-handshake.md) | active |
| MCP Serve: get_version Tool | Adds a `get_version` MCP tool (no input, returns `{"version":"x.y.z"}`) so agents can verify mid-session which binary is serving and detect stale deployments | [specifications/moeb/moeb.serve-get-version-tool.md](specifications/moeb/moeb.serve-get-version-tool.md) | active |
| Review Sub-Agent Adapter Wiring and QA Review Clarity | Fixes MCP-mode query_agent to build the configured adapter eagerly at server startup and aligns start_run.rs review-flag logic with the CLI path; rephrases run.skill.md review skip conditions to eliminate double-negative ambiguity | [specifications/moeb/moeb.review-adapter-wiring-and-qa-clarity.md](specifications/moeb/moeb.review-adapter-wiring-and-qa-clarity.md) | superseded |
| Kernel Thinness and MCP/CLI Parity Rubrics | Moves kernel-thin-and-parity from assets/rubrics/ to the project-scope .moeb/rubrics/global.rubrics.md, establishes the internal/rubrics/ vs assets/rubrics/ placement policy, and adds two trait-keyed catalogue entries: thin-kernel (kernel-change, spec+run) and mcp-cli-behavioural-parity (mcp-surface, run) | [specifications/moeb/moeb.kernel-thin-and-mcp-parity-rubrics.md](specifications/moeb/moeb.kernel-thin-and-mcp-parity-rubrics.md) | active |
| Write→Review Compliance Enforcement | Adds write-registration to `write_file`, a thin `complete_review` tool, and a `verify_rubrics` gate so that every non-`.moeb/` file write must be paired with a completed Reviewer→Moderator cycle before rubric verification passes | [specifications/moeb/moeb.write-review-compliance.md](specifications/moeb/moeb.write-review-compliance.md) | active |
| Inline Persona Review: In-Context Reviewer and Moderator Reasoning | Replaces query_agent-based review sub-loops with inline persona reasoning pre-loaded via template variables, removing the adapter dependency for review in MCP mode | [specifications/moeb/moeb.inline-persona-review.md](specifications/moeb/moeb.inline-persona-review.md) | active |
| Review Process: Patch Tool Error and Test Regression Signal Gaps | Hardens the Per-Step Review Sub-Loop and QA Architect checks to surface patch_file failures and uncertified test regression verdicts as rubric Fails and Critical signals | [specifications/moeb/moeb.review-process-patch-and-regression-signals.md](specifications/moeb/moeb.review-process-patch-and-regression-signals.md) | active |
