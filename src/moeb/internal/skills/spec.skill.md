...
1. Without calling any tool, adopt the **Retrospective Reviewer Persona** pre-loaded
   in your context. Evaluate the spec run process using the observational lenses defined
   in the Retrospective Reviewer Persona. Produce a raw JSON array of signal objects (`"severity": "Major"` or `"Minor"` only,
